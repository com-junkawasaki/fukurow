...
mod tests {
    ...
    mod handlers_routes_tests {
        ...
        use fukurow_core::model::{CyberEvent, SecurityAction};
        use fukurow_engine::{ReasonerEngine, ReasonerError};
        use fukurow_domain_cyber::threat_intelligence::{ThreatProcessor, ThreatIntelligenceError};
        use fukurow_store::GraphStore;
        use std::sync::Arc;
        ...
        #[derive(Clone)]
        struct MockReasonerEngine {
            should_fail: bool,
        }

        impl MockReasonerEngine {
            fn new() -> Self {
        ...
        #[async_trait]
        impl ReasonerEngine for MockReasonerEngine {
            async fn add_event(&self, _event: CyberEvent) -> Result<(), ReasonerError> {
                if self.should_fail {
                    return Err(ReasonerError::ReasoningError("Mock failure".to_string()));
                }
                Ok(())
            }

            async fn reason(&self) -> Result<Vec<SecurityAction>, ReasonerError> {
                if self.should_fail {
                    return Err(ReasonerError::ReasoningError("Mock failure".to_string()));
                }
                Ok(vec![SecurityAction::Alert {
                    severity: "high".to_string(),
            ...
            async fn get_graph_store(&self) -> Arc<RwLock<GraphStore>> {
                Arc::new(RwLock::new(GraphStore::new()))
            }

            async fn reset(&mut self) -> Result<(), ReasonerError> {
                Ok(())
            }

            fn get_stats(&self) -> std::collections::HashMap<String, usize> {
                let mut stats = std::collections::HashMap::new();
                stats.insert("total_events".to_string(), 1);
                stats.insert("total_actions".to_string(), 1);
                stats
            }
        }

        #[derive(Clone)]
        struct MockThreatProcessor;

        impl ThreatProcessor for MockThreatProcessor {
            fn get_statistics(&self) -> std::collections::HashMap<String, usize> {
                let mut stats = std::collections::HashMap::new();
                stats.insert("total_indicators".to_string(), 100);
                stats.insert("sources".to_string(), 5);
                stats
            }

            fn export_indicators(&self) -> Result<String, ThreatIntelligenceError> {
                Ok(r#"{"indicators": ["mock-indicator"]}"#.to_string())
            }

            fn import_indicators(&mut self, _json_data: &str) -> Result<(), ThreatIntelligenceError> {
                Ok(())
            }
        }
        ...
        fn create_test_state() -> AppState {
            AppState {
                reasoner: Arc::new(MockReasonerEngine::new()),
                threat_processor: Arc::new(RwLock::new(MockThreatProcessor)),
        ...
        #[tokio::test]
        async fn test_get_threat_intel() {
            let state = create_test_state();
            let response = get_threat_intel(State(state)).await;
            let api_response = response.0;
            assert!(api_response.success);
        }
